...
var types = map[schema.GroupVersionKind]resourcesemantics.GenericCRD{
	taskloopv1alpha1.SchemeGroupVersion.WithKind("TaskLoop"): &taskloopv1alpha1.TaskLoop{},
}
...
func newDefaultingAdmissionController(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
	...
	return defaulting.NewAdmissionController(ctx,
		...
		"webhook.taskloop.custom.tekton.dev",
...
func newValidationAdmissionController(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
	...
	return validation.NewAdmissionController(ctx,
		...
		// Name of the resource webhook.
		"validation.webhook.taskloop.custom.tekton.dev",
...
func main() {
	...
	if serviceName == "" {
		serviceName = "tekton-taskloop-webhook"
	}

	secretName := os.Getenv("WEBHOOK_SECRET_NAME")
	if secretName == "" {
		secretName = "tekton-taskloop-webhook-certs" // #nosec
	}
